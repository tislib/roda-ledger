//! Same load test as `load.rs`, but each deposit goes through a registered
//! WASM function (`Operation::Function`) instead of the built-in
//! `Operation::Deposit`. The function body is the deposit itself —
//! `debit(account, amount); credit(SYSTEM_ACCOUNT_ID, amount)` — so the work
//! per transaction matches the native path and the throughput gap is purely
//! the WASM invocation cost.
...
/// Registered name of the WASM deposit function.
const FN_NAME: &str = "wasm_deposit";

/// Deposit as a WASM function: `debit(param0, param1); credit(0, param1)`.
/// Mirrors the built-in `Operation::Deposit` (debit the account, credit the
/// system account 0), so it is zero-sum and commits.
...
#[command(
    name = "load_wasm",
    about = "WASM deposit load generator for roda-ledger"
)]
struct Args {
...
fn main() {
    ...
    ledger
        .register_function(FN_NAME, &binary, false)
        .expect("register wasm deposit function");
    ...
    println!();
    println!(
        "  wasm deposit load | accounts={} fn={} mode={}",
        account_count,
        FN_NAME,
    ...
    println!();
    println!("  ╔══════════════════════════════════════════════╗");
    println!("  ║{:^46}║", "WASM DEPOSIT LOAD SUMMARY");
    println!("  ╠══════════════════════════════════════════════╣");
    println!(
        "  ║  Duration      : {:>10.2}s                 ║",
        elapsed.as_secs_f64()
