...
fn get_within_window_returns_value() {
    ...
    for i in 2..6usize {
        assert_eq!(id_of(&ring.get(i)), i as u64);
    }
}
        ...
        .map(|_| {
            ...
            thread::spawn(move || {
                ...
                while (seen.len() as u64) < N {
                    ...
                    while local < w {
                        seen.push(id_of(&ring.get(local)));
                        local += 1;
                    }
                    std::hint::spin_loop();
                }
...
fn wrap_around() {
    ...
    for _ in 0..rounds {
        ...
        let w = ring.write_index();
        while consumed < w {
            assert_eq!(id_of(&ring.get(consumed)), consumed as u64);
            consumed += 1;
        }
...
// Collect the position ids a reader walks from `from` up to the published write_index.
