...
pub struct FollowerState {
    /// `Some(id)` after a valid handshake from `id`. Cleared only on
    /// transition out of Follower, not on heartbeat timeout — until
    /// the next leader speaks, the previous leader id remains the
    /// best guess for routing client redirects.
