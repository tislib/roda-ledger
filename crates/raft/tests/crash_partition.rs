...
use raft::{NodeId, Persistence, RaftConfig, RaftNode, RequestVote, Role};
...
/// After driver-side hydration via `advance_commit_index`, a restarted
/// node should reflect the recovered watermark.
#[test]
fn restart_recovers_local_indexes_via_advance() {
    ...
    restarted.advance_local_index(7);
    assert_eq!(restarted.commit_index(), 7);
}

/// A leader that observes a higher term steps down to follower; the
/// node-level `local_commit_index` survives the role swing.
#[test]
fn leader_step_down_preserves_commit_index() {
