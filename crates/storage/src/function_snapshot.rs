...
//!
//! Mirrors the balance snapshot ([`storage::snapshot`]) but persists the
...
//! after the snapshot.
//!
//! All public I/O on this format goes through [`storage::Storage`]
//! (`save_function_snapshot` / `load_function_snapshot`) — this module's
//! `save` / `load` helpers are `pub` so no caller outside the
...
/// [`storage::Storage::save_function_snapshot`].
pub fn save(
    data_dir: &Path,
    segment_id: u32,
    last_tx_id: u64,
...
///
/// Exposed only inside the storage layer; external callers go through
/// [`storage::Storage::load_function_snapshot`].
pub fn load(data_dir: &Path, segment_id: u32) -> io::Result<FunctionSnapshotData> {
    let bin = bin_path(data_dir, segment_id);
    let crc = crc_path(data_dir, segment_id);
