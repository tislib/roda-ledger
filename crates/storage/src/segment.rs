...
use crate::wal_reader::{read_wal_data, verify_wal_data};
...
impl Segment {
    pub fn open(data_dir: String, segment_id: u32) -> Result<Self, Error> {
    ...
    pub fn open_active(data_dir: String, segment_id: u32) -> Result<Self, Error> {
        let data_dir_path = Path::new(&data_dir);
    ...
    pub fn wal_data(&self) -> &[u8] {
        &self.wal_data
    }

    pub fn status(&self) -> SegmentStaus {
        self.status
    }

    pub fn append_pending_entry(&mut self, entry: &WalEntry) {
        assert_eq!(
            self.status,
            SegmentStaus::ACTIVE,
    ...
    pub fn write_pending_entries(&mut self) {
        assert_eq!(
            self.status,
            SegmentStaus::ACTIVE,
            "Segment is not active, cannot append"
    ...
    pub fn syncer(&self) -> Result<Syncer, Error> {
    ...
    pub fn locate_tx_position(&self, target_tx_id: u64) -> Result<Option<usize>, Error> {
        assert!(
            self.loaded,
    ...
    pub fn current_wal_offset(&self) -> usize {
        self.wal_position
    }

    pub fn wal_data_len(&self) -> usize {
        self.wal_data.len()
    }

    /// Returns a copy of the raw WAL data for external processing.
    pub fn wal_data_copy(&self) -> Vec<u8> {
        self.wal_data.clone()
    }
    ...
    pub fn create_wal_stop(data_dir: &str) -> Result<(), Error> {
    ...
    pub fn delete_wal_stop(data_dir: &str) -> Result<(), Error> {
    ...
    pub fn force_unseal(&mut self) -> Result<(), Error> {
