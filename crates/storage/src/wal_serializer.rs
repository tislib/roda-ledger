...
pub fn serialize_wal_records(entry: &WalEntry) -> &[u8] {
    match entry {
        ...
        WalEntry::KvConstant(c) => bytemuck::bytes_of(c),
    }
}
...
pub fn parse_wal_record(data: &[u8]) -> Result<WalEntry, std::io::Error> {
    ...
    match kind {
        ...
        k if k == WalEntryKind::KvConstant as u8 => {
            let c: KvConstant = bytemuck::pod_read_unaligned(record_data);
            Ok(WalEntry::KvConstant(c))
        }
...
mod tests {
    use super::*;
    use crate::entities::{
        EntryKind, FailReason, TxEntry, TxMetadata, TxTerm, WalEntry, WalEntryKind,
    };
    ...
    #[test]
    fn parse_retired_segment_header_discriminant_errors() {
        let mut data = vec![0u8; 40];
        data[0] = 2;
        assert!(parse_wal_record(&data).is_err());
    }

    #[test]
    fn parse_retired_segment_sealed_discriminant_errors() {
        let mut data = vec![0u8; 40];
        data[0] = 3;
        assert!(parse_wal_record(&data).is_err());
    }
