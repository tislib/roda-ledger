...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tx_id = client.register_transaction(WalletTransaction::deposit(account_id, deposit_amount)).await?;
    println!("Sent deposit request for account {} with amount {}. Transaction ID: {}", account_id, deposit_amount, tx_id);

    // 4. Send another transaction (transfer)
    let to_account_id = 1002;
    let transfer_amount = 2000;
    
    let tx_id_2 = client.register_transaction(WalletTransaction::transfer(account_id, to_account_id, transfer_amount)).await?;
    println!("Sent transfer request from {} to {} with amount {}. Transaction ID: {}", account_id, to_account_id, transfer_amount, tx_id_2);
