...
//! [`crate::storage::Segment`] via an `impl Segment` block in this
...
//!
//! Visibility note: this module reaches into `Segment`'s `pub(super)`
...
use crate::storage::wal_reader::read_wal_data;
...
impl Segment {
    ...
    /// tolerated.
    pub(crate) fn delete_snapshot_files_for_segment(
        data_dir: &str,
        segment_id: u32,
    ) -> Result<(), Error> {
        let dir = Path::new(data_dir);
