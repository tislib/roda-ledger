...
pub struct WalGrowthScenario {
    pub duration: Duration,
    pub accounts: u64,
}

impl WalGrowthScenario {
    pub fn new(duration: Duration, accounts: u64) -> Self {
        Self { duration, accounts }
    }
}

impl Scenario for WalGrowthScenario {
    fn name(&self) -> String {
        "wal_growth".to_string()
    }
    ...
    fn run(&self) -> Result<RunResult, Box<dyn Error>> {
        ...
        // Configure ledger with NO snapshots (very long interval)
        let config = LedgerConfig {
            snapshot_interval: Duration::from_secs(3600 * 24),
            ..Default::default()
        };
