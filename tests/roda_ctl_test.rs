...
use roda_ledger::storage::{WAL_MAGIC, WAL_VERSION};
...
fn create_test_segment(dir: &Path, segment_id: u32, tx_start: u64, num_txs: u32) -> PathBuf {
    ...
    let header = SegmentHeader {
        ...
        version: WAL_VERSION, _pad0: [0; 2], magic: WAL_MAGIC,
        segment_id, _pad1: [0; 4], first_tx_id: tx_start, _pad2: [0; 16],
    };
    ...
    for i in 0..num_txs {
        ...
        let e1 = TxEntry {
            entry_type: WalEntryKind::TxEntry as u8, kind: EntryKind::Debit,
            _pad0: [0; 6], tx_id, account_id: 1, amount: 100,
            computed_balance: 100 * (i as i64 + 1),
        };
        let e2 = TxEntry {
            entry_type: WalEntryKind::TxEntry as u8, kind: EntryKind::Credit,
            _pad0: [0; 6], tx_id, account_id: 0, amount: 100,
            computed_balance: -(100 * (i as i64 + 1)),
        };
        let mut meta = TxMetadata {
            entry_type: WalEntryKind::TxMetadata as u8, entry_count: 2,
            fail_reason: FailReason::NONE, flags: 0, crc32c: 0, tx_id,
            timestamp: 1_700_000_000 + i as u64, user_ref: 0, tag: [0; 8],
        };
    ...
    let sealed = SegmentSealed {
        entry_type: WalEntryKind::SegmentSealed as u8, _pad0: [0; 3],
        segment_id, last_tx_id, record_count, _pad1: [0; 16],
    };
...
fn unpack_produces_valid_ndjson() {
    ...
    for (i, l) in lines.iter().enumerate() {
        let _: serde_json::Value = serde_json::from_str(l)
            .unwrap_or_else(|e| panic!("line {} not JSON: {}", i, e));
    }
...
fn unpack_detects_crc_mismatch() {
    ...
    let out = roda_ctl().args(["unpack", wal.to_str().unwrap()]).output().unwrap();
...
fn unpack_ignore_crc_continues() {
    ...
    let out = roda_ctl().args(["unpack", wal.to_str().unwrap(), "--ignore-crc"]).output().unwrap();
    assert!(out.status.success());
...
fn unpack_to_file() {
    ...
    let out = roda_ctl()
        .args(["unpack", dir.join("wal_000001.bin").to_str().unwrap(), "--out", outf.to_str().unwrap()])
        .output().unwrap();
    assert!(out.status.success());
    assert!(outf.exists());
...
fn pack_round_trip() {
    ...
    roda_ctl().args(["unpack", wal.to_str().unwrap(), "--out", json.to_str().unwrap()]).output().unwrap();

    let repacked_dir = unique_dir("pack_rt_out");
    let repacked = repacked_dir.join("wal_000001.bin");
    let out = roda_ctl().args(["pack", json.to_str().unwrap(), "--out", repacked.to_str().unwrap()]).output().unwrap();
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));

    assert_eq!(original, fs::read(&repacked).unwrap(), "binary differs after round-trip");
    let _ = fs::remove_dir_all(&dir);
...
fn pack_recomputes_crc() {
    ...
    create_test_segment(&dir, 1, 1, 2);
    let json = dir.join("seg.json");
    roda_ctl().args(["unpack", dir.join("wal_000001.bin").to_str().unwrap(), "--out", json.to_str().unwrap()]).output().unwrap();
    ...
    let out = roda_ctl().args(["unpack", repacked.to_str().unwrap()]).output().unwrap();
    assert!(out.status.success(), "repacked should have valid CRCs, stderr: {}", String::from_utf8_lossy(&out.stderr));
...
"#).unwrap();
    ...
    let out = roda_ctl().args(["pack", json.to_str().unwrap(), "--out", dir.join("o.bin").to_str().unwrap()]).output().unwrap();
    assert!(!out.status.success());
...
"#).unwrap();
    ...
    let out = roda_ctl().args(["pack", json.to_str().unwrap(), "--out", dir.join("o.bin").to_str().unwrap(), "--no-validate"]).output().unwrap();
    assert!(out.status.success());
...
fn verify_succeeds_on_valid_data() {
    ...
    let out = roda_ctl().args(["verify", dir.to_str().unwrap()]).output().unwrap();
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
...
fn verify_single_segment() {
    ...
    create_test_segment(&dir, 1, 1, 5); seal_segment_files(&dir, 1);
    create_test_segment(&dir, 2, 6, 5); seal_segment_files(&dir, 2);

    let out = roda_ctl().args(["verify", dir.to_str().unwrap(), "--segment", "1"]).output().unwrap();
    assert!(out.status.success());
...
fn verify_cross_segment_tx_gap() {
    ...
    create_test_segment(&dir, 1, 1, 5); seal_segment_files(&dir, 1);
    create_test_segment(&dir, 2, 10, 5); seal_segment_files(&dir, 2); // gap
...
fn verify_range_filter() {
    ...
    create_test_segment(&dir, 1, 1, 3); seal_segment_files(&dir, 1);
    create_test_segment(&dir, 2, 4, 3); seal_segment_files(&dir, 2);
    create_test_segment(&dir, 3, 7, 3); seal_segment_files(&dir, 3);

    let out = roda_ctl().args(["verify", dir.to_str().unwrap(), "--range", "1..2"]).output().unwrap();
    assert!(out.status.success());
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("wal_000001.bin"));
    assert!(stderr.contains("wal_000002.bin"));
...
fn seal_creates_sidecar_and_marker() {
    ...
    let out = roda_ctl().args(["seal", dir.join("wal_000001.bin").to_str().unwrap()]).output().unwrap();
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
    ...
    assert_eq!(u32::from_le_bytes(crc_data[12..16].try_into().unwrap()), WAL_MAGIC);

    let wal = fs::read(dir.join("wal_000001.bin")).unwrap();
    assert_eq!(u32::from_le_bytes(crc_data[0..4].try_into().unwrap()), crc32c::crc32c(&wal));
    assert_eq!(u64::from_le_bytes(crc_data[4..12].try_into().unwrap()), wal.len() as u64);
...
fn seal_force_reseals() {
    ...
    let out = roda_ctl().args(["seal", dir.join("wal_000001.bin").to_str().unwrap(), "--force"]).output().unwrap();
    assert!(out.status.success());
...
fn seal_then_verify() {
    ...
    create_test_segment(&dir, 1, 1, 5);
    roda_ctl().args(["seal", dir.join("wal_000001.bin").to_str().unwrap()]).output().unwrap();
...
fn full_unpack_pack_seal_verify() {
    ...
    roda_ctl().args(["pack", json.to_str().unwrap(), "--out", new_bin.to_str().unwrap()]).output().unwrap();
