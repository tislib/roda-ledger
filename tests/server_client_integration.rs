...
async fn test_server_client_integration() {
    ...
    let tx_id = client.register_transaction(WalletTransaction::deposit(account_id, deposit_amount))
        .await
    ...
    // 2. Get Status
    let mut status = client.get_status(tx_id).await.expect("Failed to get status");
    ...
    while !status.balance_ready() && attempts < 10 {
        ...
        status = client.get_status(tx_id).await.expect("Failed to get status");
        attempts += 1;
    }
    
    // 3. Get Balance
    let balance = client.get_balance(account_id).await.expect("Failed to get balance");
